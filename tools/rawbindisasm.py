#!/usr/bin/env python
#
# Usage: pefdisasm.py baserom.bin
#
...
#         dataCount += 1
#     pass
entryPoint = 0x100041a0

textOffsets.append(0x0)
textAddresses.append(0x10000000)
textSizes.append(0x5b9458)
textCount += 1

dataOffsets.append(0x5b9460)
dataAddresses.append(0x105b9460)
dataSizes.append(0x7BF80)
dataCount += 1
...
def is_label_candidate(addr):
    for i in range(0, textCount):
        if addr >= textAddresses[i] and addr < textAddresses[i] + textSizes[i] and (addr & 3) == 0:
            return True
    for i in range(0, dataCount):
        if addr >= dataAddresses[i] and addr < 0x10651E10:
            return True
    return False
...
def disasm_iter(offset, address, size, callback, is_writing: bool, section: str):
    ...
    while address < end:
        ...
        if address == 0x100007A0 and is_writing:
...
def get_label_callback(address, offset, insn, bytes):
    ...
    if r2_addr != None:
        ...
        if is_load_store_reg_offset(insn, PPC_REG_R2):
            ...
            if is_label_candidate(value):
                labels.add(value)
                off = read_u32(value - 0x10000000)
                if is_label_candidate(off):
                    if off >= dataAddresses[0]:
                        labels.add(off)
                        while is_label_candidate(off):
                            if off >= textAddresses[0] and off < dataAddresses[0]:
                                labels.add(off)
                                print("# DEBUG: add label 0x%08X" % off)
                                add_label(off, 'sub_%08X' % off)
                                off = 0
                            elif off >= dataAddresses[0] and off < 0x106353E0:   
                                off = read_u32(off - 0x10000000)
                                if is_label_candidate(off):
                                    print("# DEBUG: add label 0x%08X" % off)
                                    labels.add(off)
                            else:
...
for i in range(dataAddresses[0], dataAddresses[0] + dataSizes[0]):
    if i % 4 != 0:
        continue
    value = read_u32(i - 0x10000000)
    if is_label_candidate(value) and value < 0x106353E0:
        labels.add(value)
        off = read_u32(value - 0x10000000)
        if is_label_candidate(off):
            if off >= dataAddresses[0]:
    ...
    if textSizes[i] != 0:
        ...
        while address < end:
            if j < len(sectionLabels):
                ...
                if incbinSize != 0:
                    currentFile.write("\t.incbin \"baserom.bin\", 0x%X, 0x%X\n" % (offset, incbinSize))
                l = addr_to_label(sectionLabels[j])
                currentFile.write(".global %s # 0x%08X\n%s:\n" % (l, sectionLabels[j], l))
                j += 1
            else:
...
for i in range(0, dataCount):
    ...
    if address == end: #bss
        ...
        # Split incbins by labels
        j = 0
        while address < 0x10651e04:
            if j < len(sectionLabels):
                gapSize = sectionLabels[j] - address
                if gapSize != 0:
                    currentFile.write("\t.skip 0x%X\n" % (gapSize))
                l = addr_to_label(sectionLabels[j])
                currentFile.write(".global %s # 0x%08X\n%s:\n" % (l, sectionLabels[j], l))
                j += 1
            else:
